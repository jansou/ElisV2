...
ELSlime::ELSlime
	...
	const Point& Pos
		...
		m_invisibled(false)
{
	TextureAsset::Register(L"texELSlime_stay",
		L"data/Elis/Enemy/Slime/slime_stay.png");
	TextureAsset::Register(L"texELSlime_jump",
		L"data/Elis/Enemy/Slime/slime_jump.png");
	TextureAsset::Register(L"texELSlime_damaged",
		L"data/Elis/Enemy/Slime/slime_damaged.png");
	TextureAsset::Register(L"texELSlime_dead",
...
void ELSlime::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
...
void ELSlime::attackProcess(const Point& playerpos)
{
	m_jumpH = kMaxJunpH;
...
void ELSlime::gravity(const ELMap& map, ELObjectInfo& object, const Point& camerapos)
{
...
void ELSlime::IdentifyState(const ELMap& map, ELObjectInfo& object, const Point &camerapos)
{
...
void ELSlime::MoveXAxis(const ELMap& map, int speed, ELObjectInfo& object, const Point &camerapos)
{
	m_oldState = m_state;
...
void ELSlime::jump(const ELMap& map, int jumpH, ELObjectInfo& object, const Point &camerapos)
{
...
bool ELSlime::intersectBody(const Point& foot,const Point& head, const ELMap& map)//í èÌè∞Ç∆êgëÃÇÃê⁄êGîªíË
{
...
void ELSlime::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	switch(m_state)
	{
	case ELEnemyState::Staying:
		{	
			...
			{
				TextureAsset(L"texELSlime_jump")
				(128*m_face, 128*1,	128, 128).draw(m_pos-camerapos-modifyDrawPos);
			}
			TextureAsset(L"texELSlime_stay")
			(128*m_face, 128*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELSlime_stay").height/128)), 128, 128).
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELSlime_damaged")
			(128*m_face, 0, 128, 128).draw(m_pos-camerapos-modifyDrawPos,Alpha(155));
			break;
		}
	case ELEnemyState::Dead:
		{
			TextureAsset(L"texELSlime_dead")
			(128*m_face, (m_frameCount/kAnimeTiming)%(height(L"texELSlime_dead")/128)*128, 128, 128).draw(m_pos-camerapos-modifyDrawPos);
			break;
		}
	default:
		break;
...
ELEnemySize ELSlime::getEnemySize() const
{
	return m_size;
}

Rect ELSlime::getDrawRect() const
{
	return m_drawRect;
}

Point ELSlime::getPos() const
{
	return m_pos;
}

bool ELSlime::isHit(const Rect &body) const
{
...
int ELSlime::getDamage() const
{
	return kDamage;
}

Rect ELSlime::getHitRect() const
{
	return m_hitRect;
}

void ELSlime::giveDamage(int damage)
{
...
Point ELSlime::getStartPos() const
{
	return m_startPos;
}

bool ELSlime::isErased() const
{
	return m_erased;
}

bool ELSlime::isDamaged() const
{
