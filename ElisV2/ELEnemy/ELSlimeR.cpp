...
ELSlimeR::ELSlimeR
	...
	const Point& Pos
		...
		m_invisibled(false)
{
	TextureAsset::Register(L"texELSlimeR_stay",
		L"data/Elis/Enemy/SlimeR/slime_stay.png");
	TextureAsset::Register(L"texELSlimeR_jump",
		L"data/Elis/Enemy/SlimeR/slime_jump.png");
	TextureAsset::Register(L"texELSlimeR_damaged",
		L"data/Elis/Enemy/SlimeR/slime_damaged.png");
	TextureAsset::Register(L"texELSlimeR_dead",
		L"data/Elis/Enemy/SlimeR/slime_dead.png");
}
...
void ELSlimeR::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
...
void ELSlimeR::attackProcess(const Point& playerpos)
{
	m_jumpH = kMaxJunpH;
...
void ELSlimeR::gravity(const ELMap& map, ELObjectInfo& object, const Point& camerapos)
{
...
void ELSlimeR::IdentifyState(const ELMap& map, ELObjectInfo& object, const Point &camerapos)
{
...
void ELSlimeR::MoveXAxis(const ELMap& map, int speed, ELObjectInfo& object, const Point &camerapos)
{
	m_oldState = m_state;
...
void ELSlimeR::jump(const ELMap& map, int jumpH, ELObjectInfo& object, const Point &camerapos)
{
...
bool ELSlimeR::intersectBody(const Point& foot,const Point& head, const ELMap& map)//í èÌè∞Ç∆êgëÃÇÃê⁄êGîªíË
{
...
void ELSlimeR::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	switch(m_state)
	{
	case ELEnemyState::Staying:
		{	
			...
			{
				TextureAsset(L"texELSlimeR_jump")
				(128*m_face, 128*1,	128, 128).draw(m_pos-camerapos-modifyDrawPos);
			}
			TextureAsset(L"texELSlimeR_stay")
			(128*m_face, 128*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELSlimeR_stay").height/128)), 128, 128).
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELSlimeR_damaged")
			(128*m_face, 0, 128, 128).draw(m_pos-camerapos-modifyDrawPos,Alpha(155));
			break;
		}
	case ELEnemyState::Dead:
		{
			TextureAsset(L"texELSlimeR_dead")
			(128*m_face, (m_frameCount/kAnimeTiming)%(height(L"texELSlimeR_dead")/128)*128, 128, 128).draw(m_pos-camerapos-modifyDrawPos);
			break;
		}
	default:
		break;
...
ELEnemySize ELSlimeR::getEnemySize() const
{
	return m_size;
}

Rect ELSlimeR::getDrawRect() const
{
	return m_drawRect;
}

Point ELSlimeR::getPos() const
{
	return m_pos;
}

bool ELSlimeR::isHit(const Rect &body) const
{
...
int ELSlimeR::getDamage() const
{
	return kDamage;
}

Rect ELSlimeR::getHitRect() const
{
	return m_hitRect;
}

void ELSlimeR::giveDamage(int damage)
{
...
Point ELSlimeR::getStartPos() const
{
	return m_startPos;
}

bool ELSlimeR::isErased() const
{
	return m_erased;
}

bool ELSlimeR::isDamaged() const
{
