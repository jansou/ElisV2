...
ELSlimeD::ELSlimeD
	...
	const Point& Pos
		...
		m_invisibled(false)
{
	TextureAsset::Register(L"texELSlimeD_stay",
		L"data/Elis/Enemy/SlimeD/slime_stay.png");
	TextureAsset::Register(L"texELSlimeD_jump",
		L"data/Elis/Enemy/SlimeD/slime_jump.png");
	TextureAsset::Register(L"texELSlimeD_damaged",
		L"data/Elis/Enemy/SlimeD/slime_damaged.png");
	TextureAsset::Register(L"texELSlimeD_dead",
		L"data/Elis/Enemy/SlimeD/slime_dead.png");
}
...
void ELSlimeD::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
...
void ELSlimeD::attackProcess(const Point& playerpos)
{
	m_jumpH = kMaxJunpH;
...
void ELSlimeD::gravity(const ELMap& map, ELObjectInfo& object, const Point& camerapos)
{
...
void ELSlimeD::IdentifyState(const ELMap& map, ELObjectInfo& object, const Point &camerapos)
{
...
void ELSlimeD::MoveXAxis(const ELMap& map, int speed, ELObjectInfo& object, const Point &camerapos)
{
	m_oldState = m_state;
...
void ELSlimeD::jump(const ELMap& map, int jumpH, ELObjectInfo& object, const Point &camerapos)
{
...
bool ELSlimeD::intersectBody(const Point& foot,const Point& head, const ELMap& map)//í èÌè∞Ç∆êgëÃÇÃê⁄êGîªíË
{
...
void ELSlimeD::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	switch(m_state)
	{
	case ELEnemyState::Staying:
		{	
			...
			{
				TextureAsset(L"texELSlimeD_jump")
				(128*m_face, 128*1,	128, 128).draw(m_pos-camerapos-modifyDrawPos);
			}
			TextureAsset(L"texELSlimeD_stay")
			(128*m_face, 128*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELSlimeD_stay").height/128)), 128, 128).
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELSlimeD_damaged")
			(128*m_face, 0, 128, 128).draw(m_pos-camerapos-modifyDrawPos,Alpha(155));
			break;
		}
	case ELEnemyState::Dead:
		{
			TextureAsset(L"texELSlimeD_dead")
			(128*m_face, (m_frameCount/kAnimeTiming)%(height(L"texELSlimeD_dead")/128)*128, 128, 128).draw(m_pos-camerapos-modifyDrawPos);
			break;
		}
	default:
		break;
...
ELEnemySize ELSlimeD::getEnemySize() const
{
	return m_size;
}

Rect ELSlimeD::getDrawRect() const
{
	return m_drawRect;
}

Point ELSlimeD::getPos() const
{
	return m_pos;
}

bool ELSlimeD::isHit(const Rect &body) const
{
...
int ELSlimeD::getDamage() const
{
	return kDamage;
}

Rect ELSlimeD::getHitRect() const
{
	return m_hitRect;
}

void ELSlimeD::giveDamage(int damage)
{
...
Point ELSlimeD::getStartPos() const
{
	return m_startPos;
}

bool ELSlimeD::isErased() const
{
	return m_erased;
}

bool ELSlimeD::isDamaged() const
{
