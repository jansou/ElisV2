...
ELSlimeW::ELSlimeW
	...
	const Point& Pos
		...
		m_invisibled(false)
{
	TextureAsset::Register(L"texELSlimeW_stay",
		L"data/Elis/Enemy/SlimeW/slime_stay.png");
	TextureAsset::Register(L"texELSlimeW_jump",
		L"data/Elis/Enemy/SlimeW/slime_jump.png");
	TextureAsset::Register(L"texELSlimeW_damaged",
		L"data/Elis/Enemy/SlimeW/slime_damaged.png");
	TextureAsset::Register(L"texELSlimeW_dead",
		L"data/Elis/Enemy/SlimeW/slime_dead.png");
}
...
void ELSlimeW::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
...
void ELSlimeW::attackProcess(const Point& playerpos)
{
	m_jumpH = kMaxJunpH;
...
void ELSlimeW::gravity(const ELMap& map, ELObjectInfo& object, const Point& camerapos)
{
...
void ELSlimeW::IdentifyState(const ELMap& map, ELObjectInfo& object, const Point &camerapos)
{
...
void ELSlimeW::MoveXAxis(const ELMap& map, int spd, ELObjectInfo& object, const Point &camerapos)
{
	m_oldState = m_state;

	const int dir = (0<=spd ? 1 : -1);

	for(int i=0;i<std::abs(spd);++i)
	{
...
void ELSlimeW::jump(const ELMap& map, int jumpH, ELObjectInfo& object, const Point &camerapos)
{
...
bool ELSlimeW::intersectBody(const Point& foot,const Point& head, const ELMap& map)//í èÌè∞Ç∆êgëÃÇÃê⁄êGîªíË
{
...
void ELSlimeW::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	switch(m_state)
	{
	case ELEnemyState::Staying:
		{	
			...
			{
				TextureAsset(L"texELSlimeW_jump")
				(128*m_face, 128*1,	128, 128).draw(m_pos-camerapos-modifyDrawPos);
			}
			TextureAsset(L"texELSlimeW_stay")
			(128*m_face, 128*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELSlimeW_stay").height/128)), 128, 128).
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELSlimeW_damaged")
			(128*m_face, 0, 128, 128).draw(m_pos-camerapos-modifyDrawPos,Alpha(155));
			break;
		}
	case ELEnemyState::Dead:
		{
			TextureAsset(L"texELSlimeW_dead")
			(128*m_face, (m_frameCount/kAnimeTiming)%(height(L"texELSlimeW_dead")/128)*128, 128, 128).draw(m_pos-camerapos-modifyDrawPos);
			break;
		}
	default:
		break;
...
ELEnemySize ELSlimeW::getEnemySize() const
{
	return m_size;
}

Rect ELSlimeW::getDrawRect() const
{
	return m_drawRect;
}

Point ELSlimeW::getPos() const
{
	return m_pos;
}

bool ELSlimeW::isHit(const Rect &body) const
{
...
int ELSlimeW::getDamage() const
{
	return kDamage;
}

Rect ELSlimeW::getHitRect() const
{
	return m_hitRect;
}

void ELSlimeW::giveDamage(int damage)
{
...
Point ELSlimeW::getStartPos() const
{
	return m_startPos;
}

bool ELSlimeW::isErased() const
{
	return m_erased;
}

bool ELSlimeW::isDamaged() const
{
