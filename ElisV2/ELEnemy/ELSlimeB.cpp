...
ELSlimeB::ELSlimeB
	...
	const Point& Pos
		...
		m_invisibled(false)
{
	TextureAsset::Register(L"texELSlimeB_stay",
		L"data/Elis/Enemy/SlimeB/slime_stay.png");
	TextureAsset::Register(L"texELSlimeB_jump",
		L"data/Elis/Enemy/SlimeB/slime_jump.png");
	TextureAsset::Register(L"texELSlimeB_damaged",
		L"data/Elis/Enemy/SlimeB/slime_damaged.png");
	TextureAsset::Register(L"texELSlimeB_dead",
		L"data/Elis/Enemy/SlimeB/slime_dead.png");
}
...
void ELSlimeB::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
...
void ELSlimeB::attackProcess(const Point& playerpos)
{
	m_jumpH = kMaxJunpH;
...
void ELSlimeB::gravity(const ELMap& map, ELObjectInfo& object, const Point& camerapos)
{
...
void ELSlimeB::IdentifyState(const ELMap& map, ELObjectInfo& object, const Point &camerapos)
{
...
void ELSlimeB::MoveXAxis(const ELMap& map, int speed, ELObjectInfo& object, const Point &camerapos)
{
	m_oldState = m_state;
...
void ELSlimeB::jump(const ELMap& map, int jumpH, ELObjectInfo& object, const Point &camerapos)
{
...
bool ELSlimeB::intersectBody(const Point& foot,const Point& head, const ELMap& map)//í èÌè∞Ç∆êgëÃÇÃê⁄êGîªíË
{
...
void ELSlimeB::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	switch(m_state)
	{
	case ELEnemyState::Staying:
		{	
			...
			{
				TextureAsset(L"texELSlimeB_jump")
				(128*m_face, 128*1,	128, 128).draw(m_pos-camerapos-modifyDrawPos);
			}
			TextureAsset(L"texELSlimeB_stay")
			(128*m_face, 128*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELSlimeB_stay").height/128)), 128, 128).
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELSlimeB_damaged")
			(128*m_face, 0, 128, 128).draw(m_pos-camerapos-modifyDrawPos,Alpha(155));
			break;
		}
	case ELEnemyState::Dead:
		{
			TextureAsset(L"texELSlimeB_dead")
			(128*m_face, (m_frameCount/kAnimeTiming)%(height(L"texELSlimeB_dead")/128)*128, 128, 128).draw(m_pos-camerapos-modifyDrawPos);
			break;
		}
	default:
		break;
...
ELEnemySize ELSlimeB::getEnemySize() const
{
	return m_size;
}

Rect ELSlimeB::getDrawRect() const
{
	return m_drawRect;
}

Point ELSlimeB::getPos() const
{
	return m_pos;
}

bool ELSlimeB::isHit(const Rect &body) const
{
...
int ELSlimeB::getDamage() const
{
	return kDamage;
}

Rect ELSlimeB::getHitRect() const
{
	return m_hitRect;
}

void ELSlimeB::giveDamage(int damage)
{
...
Point ELSlimeB::getStartPos() const
{
	return m_startPos;
}

bool ELSlimeB::isErased() const
{
	return m_erased;
}

bool ELSlimeB::isDamaged() const
{
