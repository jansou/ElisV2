# include "ELMinotaur.h"


ELMinotaur::ELMinotaur
	(
...
{
	//stay/////////////////////////////////////////////
	TextureAsset::Register(L"texELMinotaur_stay",
		L"data/Elis/Enemy/Minotaur/minotaur_stay.png");
	TextureAsset::Register(L"texELMinoArm_stay",
		L"data/Elis/Enemy/Minotaur/minoArm_stay.png");

	//
	TextureAsset::Register(L"texELMinotaur_damaged",
		L"data/Elis/Enemy/Minotaur/minotaur_damaged.png");
	TextureAsset::Register(L"texELMinotaur_dead",
		L"data/Elis/Enemy/Minotaur/minotaur_dead.png");
}
...
void ELMinotaur::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
...
void ELMinotaur::gravity(const ELMap& map, ELObjectInfo& object, const Point &camerapos)
{
...
void ELMinotaur::IdentifyState(const ELMap& map, ELObjectInfo& object)
{
	m_state = ELEnemyState::Staying;
	return;
}

void ELMinotaur::MoveXAxis(const ELMap& map, int speed, const Point &camerapos)
{
	m_oldState = m_state;
...
bool ELMinotaur::intersectBody(const Point& foot,const Point& head, const ELMap& map)//’Êí°‚Æg‘Ì‚ÌÚG”»’è
{
...
void ELMinotaur::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	switch(m_state)
	{
	case ELEnemyState::Staying:
		{
		int offset = (m_frameCount / 10) % 3;

			if (m_face == ELEnemyFace::EnemyLeft)
			{
				TextureAsset(L"texELMinotaur_stay")
					.mirror()
					.draw(m_pos - camerapos - modifyDrawPos);

				TextureAsset(L"texELMinoArm_stay")
					.mirror()
					.draw(m_pos - camerapos - modifyDrawPos + Point(16, 27 + offset)/* + Point(56, 27)*/);
			}
			else if (m_face == ELEnemyFace::EnemyRight)
			{
				...
				TextureAsset(L"texELMinoArm_stay")
					.draw(m_pos - camerapos - modifyDrawPos + Point(72, 27 + offset));
			}
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELMinotaur_damaged")
				(256*m_face,
				0,
				256,
	...
	case ELEnemyState::Dead:
		{
			TextureAsset(L"texELMinotaur_dead")
			(256*m_face, (m_frameCount/kAnimeTiming)%(height(L"texELMinotaur_dead")/256)*256, 256, 256).draw(m_pos-camerapos-modifyDrawPos);
			break;
		}
	default:
		break;
...
ELEnemySize ELMinotaur::getEnemySize() const
{
	return m_size;
}

Point ELMinotaur::getPos() const
{
	return m_pos;
}

bool ELMinotaur::isHit(const Rect &body) const
{
...
int ELMinotaur::getDamage() const
{
	return kDamage;
}

Rect ELMinotaur::getHitRect() const
{
	return m_hitRect;
}

void ELMinotaur::giveDamage(int damage)
{
...
Point ELMinotaur::getStartPos() const
{
	return m_startPos;
}

bool ELMinotaur::isErased() const
{
	return m_erased;
}

bool ELMinotaur::isDamaged() const
{
