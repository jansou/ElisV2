# include "ELAirsnake.h"


ELAirsnake::ELAirsnake
	(
		...
		m_erased(false)
{
	...
	TextureAsset::Register(L"texELAirsnake_stay",
		L"data/Elis/Enemy/Airsnake/airsnake_stay.png");
	TextureAsset::Register(L"texELAirsnake_damaged",
		L"data/Elis/Enemy/Airsnake/airsnake_damaged.png");
	TextureAsset::Register(L"texELAirsnake_attack",
		L"data/Elis/Enemy/Airsnake/airsnake_attack.png");
	/*
	TextureAsset::Register(L"texELAirsnake_dead",
		L"data/Elis/Enemy/Airsnake/airsnake_dead.png");
		*/

	SoundAsset::Register(L"sndELAirsnake_fire",
...
void ELAirsnake::update(const ELMap& map, const Point& playerpos, ELObjectInfo& object, ELItem &item, ELAttack &attack, const Point &camerapos)
{
	...
	if(m_state == ELEnemyState::Attacking)
	{
		...
		else if(m_frameCount == kAnimeTiming*3)
		{
			...
			if(SoundAsset(L"sndELAirsnake_fire").isPlaying)	SoundAsset(L"sndELAirsnake_fire").stop();
			
			SoundAsset(L"sndELAirsnake_fire").play();
		}
	}
...
void ELAirsnake::IdentifyState(const ELMap& map, ELObjectInfo& object)
{
	m_state = ELEnemyState::Staying;
	return;
}

void ELAirsnake::MoveXAxis(const ELMap& map, int speed, const Point &camerapos, ELObjectInfo& object)
{
	m_oldState = m_state;
...
void ELAirsnake::MoveYAxis(const ELMap& map, int speed, const Point &camerapos, ELObjectInfo& object)
{
	const int dir = (0<=speed ? 1 : -1);
...
bool ELAirsnake::intersectBody(const Point& foot,const Point& head, const ELMap& map)//í èÌè∞Ç∆êgëÃÇÃê⁄êGîªíË
{
...
void ELAirsnake::draw(const Point& camerapos, const Point& modifyDrawPos)const
{
	const int kw = 128;
	const int kh = 128;
	...
	case ELEnemyState::Staying:
		{	
			TextureAsset(L"texELAirsnake_stay")
			(
				kw*m_face,
				kh*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELAirsnake_stay").height/kh)),
				kw,
				kh
	...
	case ELEnemyState::Attacking:
		{	
			TextureAsset(L"texELAirsnake_attack")
			(
				kw*m_face,
				kh*(m_frameCount/kAnimeTiming%(TextureAsset(L"texELAirsnake_attack").height/kh)),
				kw,
				kh
				).draw(m_pos-camerapos-modifyDrawPos);
	...
	case ELEnemyState::Damaged:
		{
			TextureAsset(L"texELAirsnake_damaged")
				(kw*m_face,
				0,
				kw,
...
ELEnemySize ELAirsnake::getEnemySize() const
{
	return m_size;
}

Point ELAirsnake::getPos() const
{
	return m_pos;
}

bool ELAirsnake::isHit(const Rect &body) const
{
...
int ELAirsnake::getDamage() const
{
	return kDamage;
}

Rect ELAirsnake::getHitRect() const
{
	return m_hitRect;
}

void ELAirsnake::giveDamage(int damage)
{
...
Point ELAirsnake::getStartPos() const
{
	return m_startPos;
}

bool ELAirsnake::isErased() const
{
	return m_erased;
}

bool ELAirsnake::isDamaged() const
{
