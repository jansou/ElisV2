...
namespace
{
	...
	const int kDamage = 3;//ÚG‚Ìƒ_ƒ[ƒW
...
bool ELSeaserpent::intersectBody(const Point& foot,const Point& head, const ELMap& map)//’Êí°‚Æg‘Ì‚ÌÚG”»’è
{
...
Rect ELSeaserpent::getHitRect() const//’e‚É‚½‚¢‚µ‚Ä‚Ì“–‚½‚è”»’è‚Ég‚¤
{
	return m_hitRect;
}

void ELSeaserpent::giveDamage(int damage)//‚±‚Ì“G‚Éƒ_ƒ[ƒW‚ğ—^‚¦‚é
{
...
Point ELSeaserpent::getStartPos() const//‰ŠúˆÊ’u‚Ìæ“¾
{
	return m_startPos;
}

bool ELSeaserpent::isErased() const//Á–Å‚µ‚Ä‚à‚æ‚¢‚©
{
	return m_erased;
}

bool ELSeaserpent::isDamaged() const//ƒ_ƒ[ƒW‚ğH‚ç‚Á‚Ä‚¢‚é‚©
{
...
bool ELSeaserpentTentacle::intersectBody(const Point& foot, const Point& head, const ELMap& map)//’Êí°‚Æg‘Ì‚ÌÚG”»’è
{
	for (int i = 1; i<foot.y - head.y; ++i)
	{
		if (map.isFloor(foot.x, foot.y - i) && map.getFloorType(foot.x, foot.y - i) == ELFloorType::NormalFloor)
