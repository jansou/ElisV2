# include "ELElisShoot2.h"


ELElisShoot2::ELElisShoot2(
...
void ELElisShoot2::update(const ELMap& map)
{
	if(m_collided)
	{
...
void ELElisShoot2::draw(const Point& camerapos)const
{
	//LOGFMT_ERR("shoot!!!!!");
	...
	if(m_collided)
	{
		TextureAsset(L"texElisShot2_erase")(
			0,(m_frameCount/kAnimeTiming)%(height(L"texElisShot2_erase")/128)*128-1,128,128
			).drawAt(m_hitRect.center-camerapos);
		//Circle(m_pos.x-camerapos.x,m_pos.y-camerapos.y,10).draw(Palette::Black);
		return;
	}

	TextureAsset(L"texElisShot2")(kx,(m_frameCount/kAnimeTiming)%(height(L"texElisShot2")/128)*128+ky,kw,kh).
			rotate(
...
Point ELElisShoot2::getPos() const
{
	return Point(int(m_pos.x),int(m_pos.y));
}

bool ELElisShoot2::isHit(const Rect& body)
{
	if(!m_collided)
	{
		if(body.intersects( 
...
ELShooter ELElisShoot2::getShooter() const
{
	return m_shooter;
}

int ELElisShoot2::getDamage() const
{
	return m_damege;
}

bool ELElisShoot2::isErased() const
{
	return m_erased;
}

ELShootType ELElisShoot2::getShootType() const
{
	return m_shootType;
}

void ELElisShoot2::giveErase()
{
	m_collided = true;
}