...
void ELElisIce::update(const ELMap& map)
{
...
void ELElisIce::draw(const Point& camerapos)const
{
	//LOGFMT_ERR("shoot!!!!!");
...
Point ELElisIce::getPos() const
{
	return Point(int(m_pos.x),int(m_pos.y));
}

bool ELElisIce::isHit(const Rect& body)
{
...
ELShooter ELElisIce::getShooter() const
{
	return m_shooter;
}

int ELElisIce::getDamage() const
{
	return m_damege;
}

bool ELElisIce::isErased() const
{
	return m_erased;
}

ELShootType ELElisIce::getShootType() const
{
	return m_shootType;
}

void ELElisIce::giveErase()
{
	m_collided = true;
}