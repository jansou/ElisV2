...
class ELRockfall : public ELShoot
{
private:
	int m_frameCount;
	int m_oldFrameCount;
	...
	void playEraseSound() const
	{
		if(SoundAsset(L"sndELBlockBreak").isPlaying)
			SoundAsset(L"sndELBlockBreak").stop();

		SoundAsset(L"sndELBlockBreak").play();
	}

public:
	ELRockfall(const ELShootType& shootType,
